...
bool semaphore::pop_a_waiter(internal::routine* current_routine) {
  ...
  if (waiters_.pop(waiter) && waiter) {
    ...
    thread* managing_thread = waiter->thread_;
      boson::debug::log("Routine {}:{} to unlock itself" ,current->id(), current_routine->id() ,managing_thread->id(), waiter->id());
    if (current_routine->id() == waiter->id()) {
      waiter.release();
      return false;
    }
    else {
      boson::debug::log("Routine {}:{} to unlock {}:{}" ,current->id(), current_routine->id() ,managing_thread->id(), waiter->id());
...
void semaphore::wait() {
  ...
  while (result <= 0) {
    ...
    boson::debug::log("Routine {}:{} failed to take the lock {}",this_thread->id(), current_routine->id(),reinterpret_cast<size_t>(this));
    ...
    if (suspend) {
      ...
      boson::debug::log("Routine {}:{} suspended on wait",this_thread->id(), current_routine->id());
